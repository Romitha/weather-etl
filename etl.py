...
from schema.data_class import WeatherData, Forecast, CurrentWeather, Location
import os
...
class WeatherPipeline:
    def __init__(self, json_file: str):
        ...
        self.THRESHOLDS = {
            'high_temp': float(os.getenv('HIGH_TEMP')),
            'heavy_rain': float(os.getenv('HEAVY_RAIN')),
            'strong_wind': float(os.getenv('STRONG_WIND'))
        }
    ...
    def create_visualizations(self, output_dir='output/'):
        ...
        try:
            ...
            for city in self.forecast_df['city'].unique():
                city_data = self.forecast_df[self.forecast_df['city'] == city]
                plt.plot(city_data['date'], city_data['max_temp_c'], 
                        label=city, marker='o', linewidth=2, markersize=6)
            ...
            scatter = plt.scatter(self.temp_analysis_df['current_temp'],
                                self.temp_analysis_df['highest_temp'],
                                alpha=0.7, s=100)
            ...
            max_temp = max(self.temp_analysis_df['current_temp'].max(), 
                        self.temp_analysis_df['highest_temp'].max())
            plt.plot([0, max_temp], [0, max_temp], '--', 
                    color='gray', label='Current = Highest',
    ...
    def analyze_temperatures(self):
        ...
        try:
            ...
            for city in self.current_weather_df['city'].unique():
                current_temp = self.current_weather_df[self.current_weather_df['city'] == city]['temperature_c'].values[0]
        ...
        try:
            ...
            if not self.alerts_df.empty:
                ...
                for _, alert in self.alerts_df.iterrows():
                    report += (f"City: {alert['city']}, Date: {alert['date']}, "
